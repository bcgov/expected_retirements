...
-   This app predicts British Columbia "retirement" numbers based on demographic data, both observed and predicted. 
-   The definition of retirement is broad: permanent exit from the labour market, whether above or below ground.
-   The main driver of retirements is the number of individuals of each age/sex combination.
-   For the app the user specifies:
    1)  the labour force participation rate, determining the approximate number of labour force participants at risk of retirement.
    2)  hazard rates for retirement, which is the probability of retirement in the current year, conditional on "survival" i.e. you have not previously retired.
...
observeEvent(input$reset, {
  updateSliderInput(inputId = "male_part_rate", value = .9)
  updateSliderInput(inputId = "female_part_rate", value = .8)
...
retire_distribution <- reactive({
  Age <-55:71
  `Hazard Rate` <- c(rep(input$hazard_55_57, 3),
...
reported <- reported%>%
  ...
  mutate(`Both Sexes`=Males+Females)%>%
  pivot_longer(cols=-ref_date,names_to = "sex", values_to = "reported retirements")
...
plt <- ggplot(retire_distribution(), aes(x=factor(Age),
  ...
  labs(title="Joint probability distribution of retirement age",
    x="Age",
       y="")
plotly::ggplotly(plt, tooltip = "text")%>% 
  config(displayModeBar = F)
