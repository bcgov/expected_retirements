...
- grey horizontal lines represent the retirement filter.
...
```{r}
plt <- ggplot(participation,aes(Date,`Participation Rate`,colour=Sex))+
...
```{r}
renderPlot({
ggplot(retire_distribution(), aes(x=factor(age), y=retire_at_age))+
  geom_col()+
...
```{r}
renderPlot({
ggplot(both(), aes(ref_date, value, colour=name))+
  geom_line()+
...
-   Reported retirements from Statistics Canada table 14-10-0126-01, population data from Statistics Canada table 17-10-0005-01 and projections from <https://bcstats.shinyapps.io/popApp/>

-   Things to try:
  1) Simulate a policy intervention where the government imposes a minimum retirement age of 66 (set the hazard rate to zero for 65 and younger). What impact does this have on the expected retirements curves?
